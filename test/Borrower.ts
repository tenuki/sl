...
describe("Borrower", function () {
  ...
  async function myDeployOneYearLockFixture() {
    ...
    const tokenA = await ethers.deployContract("SomeToken", ["TokenA", "TKA"]);
    const tokenB = await ethers.deployContract("SomeToken", [ "TokenB", "TKB"]);
    tokenA.attach()
    const smartAcc = await hre.viem.deployContract("Borrower", [0],
                                                    {value: 0,});

    const publicClient = await hre.viem.getPublicClient();

    //const TokenA = IERC20(tokenA);
  ...
  describe("My Borrower Deployment", function () {
    it("Setup", async function () {
      const { otherAccount, owner } = await loadFixture(myDeployOneYearLockFixture);
